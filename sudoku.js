...
applyHubTheme();
/* =======================================================
   🎮 【C-2】ポーズ機能 & 【C-1】ベストタイム記録 & 【A-5】クリア演出
   ======================================================= */

// 1. 【C-2】ポーズ画面の表示/非表示切り替え
...
function quitToMenu() {
    ...
    // メニューに戻ったら最新のベストタイムを再表示
    updateBestTimeDisplay();
}

// 3. 【C-1】ベストタイムの保存（他ゲーム一括管理用キー: CLAN_GAME_SCORES）
function saveBestTime(difficulty, timeInSeconds) {
    let allScores = JSON.parse(localStorage.getItem('CLAN_GAME_SCORES') || '{}');
    if (!allScores.sudoku) allScores.sudoku = {};

    const currentBest = allScores.sudoku[difficulty] || Infinity;
    let isNewRecord = false;

    if (timeInSeconds < currentBest) {
        allScores.sudoku[difficulty] = timeInSeconds;
        localStorage.setItem('CLAN_GAME_SCORES', JSON.stringify(allScores));
        isNewRecord = true;
    }
    return isNewRecord;
}

// 4. メニュー画面にベストタイムを表示する処理
function updateBestTimeDisplay() {
    const display = document.getElementById('best-time-display');
    if (!display) return;

    let allScores = JSON.parse(localStorage.getItem('CLAN_GAME_SCORES') || '{}');
    if (allScores.sudoku && allScores.sudoku.easy) {
        const sec = allScores.sudoku.easy;
        const minStr = String(Math.floor(sec / 60)).padStart(2, '0');
        const secStr = String(sec % 60).padStart(2, '0');
        display.textContent = `BEST TIME: ${minStr}:${secStr}`;
    } else {
        display.textContent = `BEST TIME: --:--`;
    }
}

// 5. 【A-5】クリア時に呼び出す処理（数独完成時の処理内で呼び出してください）
function showClearModal(difficulty, clearTimeInSeconds, formattedTimeText) {
    // タイムを保存し、新記録かどうか判定
    const isNew = saveBestTime(difficulty, clearTimeInSeconds);

    // モーダル内のテキスト更新
    document.getElementById('clear-time-text').textContent = `TIME: ${formattedTimeText}`;
    const newRecordTag = document.getElementById('new-record-tag');
    if (newRecordTag) {
        newRecordTag.style.display = isNew ? 'block' : 'none';
    }
...
// ページ読み込み時にベストタイム表示を更新
document.addEventListener('DOMContentLoaded', updateBestTimeDisplay);
